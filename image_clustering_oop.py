...
class imgcls:
	...
	def clustering_gmm(self):
		clf = mixture.GaussianMixture(n_components=self.n_clusters, covariance_type='full')
		clf.fit(self.images_new)
		predictions =clf.predict(self.images_new)
		for i in range(self.max_examples):
			shutil.copy2(self.folder_path+"\\"+self.image_paths[i], "output\cluster"+str(predictions[i]))
		print("\n Clustering complete! \n\n Clusters and the respective images are stored in the \"output\" folder.")
...
if __name__ == "__main__":
	...
	temp = imgcls(data_path, number_of_clusters, max_examples, use_imagenets, use_pca)
	temp.load_images()
	temp.get_new_imagevectors()
	temp.clustering_kmeans()
#    temp.clustering_gmm()
